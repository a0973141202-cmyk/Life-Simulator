...
export function isDossierLeakSentence(sentence) {
  const text = String(sentence || "").trim();
  if (!text) return true;
  return DOSSIER_LEAK_RE.test(text);
}

export function isMetaPublicSentence(sentence) {
  const text = String(sentence || "").trim();
  if (!text) return true;
  return META_PHRASE_RE.test(text) || isDossierLeakSentence(text);
}

...
export function ageBand(ageYears = 0) {
  ...
  if (age <= STUDENT_MAX) return AGE_BANDS.student;
...
export function contentAllowedForAge(item, ageOrCtx = 0, maybeCtx = null) {
  ...
  if (age <= STUDENT_MAX && String(lane).startsWith("adult_")) return false;
...
export function dailyAudienceAllowed(audience, ageYears = 0) {
  ...
  if (age <= 12) return audience === "child_safe";
  if (age <= STUDENT_MAX) return audience === "teen";
  if (!audience || audience === "span") return true;
  const bounds = {
    child_safe: { min: 5, max: 12 },
    teen: { min: 13, max: 17 },
    teen_up: { min: 18, max: 120 },
    gray: { min: 18, max: 120 },
    adult: { min: 18, max: 120 },
  }[audience] || { min: 18, max: 120 };
